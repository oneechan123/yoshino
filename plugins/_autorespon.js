...
Pembayaran bisa melalui: Gopay, Dana , Pulsa (By U)
...
`, watermark, 'Owner', '.creator', 'Sewa', '.sewa', 'Join', '.join', m)
    ...
    if (setting.statusUpdate) {
        if (new Date() * 1 - setting.status > 1000) {
            ...
            await this.setStatus(`Aktif selama ${uptime} | Mode: ${settings.self ? 'Private' : settings.group ? 'Hanya Grup' : 'Publik'} | Bantu Donasi Supaya Bot On Terus`).catch(_ => _)
